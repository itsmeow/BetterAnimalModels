...
import dev.itsmeow.betteranimalmodels.compat.QuarkUtil;
import dev.itsmeow.betteranimalmodels.compat.QuarkUtil.VariantTextureType;
...
public class Replacements {
    ...
    public static void addReplaces() {
        ...
        H.addAction(RegistrationTime.CLIENTSETUP, "minecraft", () -> () -> {
            ...
            if(!quarkLoaded || !BetterAnimalModelsConfig.enableQuarkOverrideCow.get()) {
                H.addReplace(RegistrationTime.CLIENTSETUP, "minecraft", "cow", () -> () -> H.lambdaReplace(EntityType.COW, 0.7F, cow));
            }
            if(!quarkLoaded || !BetterAnimalModelsConfig.enableQuarkOverridePig.get()) {
                H.addReplace(RegistrationTime.CLIENTSETUP, "minecraft", "pig", () -> () -> H.lambdaReplace(EntityType.PIG, 0.7F, pig));
            }
            if(!quarkLoaded || !BetterAnimalModelsConfig.enableQuarkOverrideChicken.get()) {
